...
const SubjectDetails = ({ subject, onNavigate }) => {
  ...
  const flattenChapters = (col) =>
    col.flat().map(([id, chapter], idx) => ({
      ...
      chapter,
      letter:
        letterPrefixes[chapterEntries.findIndex(([cid]) => cid === id)] || "?",
...
function SubjectsPage() {
  ...
  const [subjectsData, setSubjectsData] = useState(chapters);
  const [selectedSubject, setSelectedSubject] = useState(chapters[0]);
  ...
  const onNavigate = useCallback(
    (subjectName, lesson) => {
      const encodedSkill = encodeURIComponent(lesson.skill);
      console.log(encodedSkill);
      navigate(`/quiz/${encodeURIComponent(subjectName)}/${encodedSkill}`);
    },
    [navigate]
  );
