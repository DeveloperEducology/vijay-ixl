...
const YouTubePlayer = ({ videoId }) => (
  <div className="w-full h-full bg-black">
    <iframe src={`https://www.youtube.com/embed/${videoId}?autoplay=1&mute=0&controls=1&rel=0&loop=1&playlist=${videoId}`} frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen title="Embedded youtube" className="w-full h-full" />
  </div>
);
...
const NewsShort = ({ article, isActive, isPrevious, swipeDirection }) => {
  ...
  const getAnimationClass = () => {
    ...
    if (isPrevious) { // Card animating OUT
  ...
  return (
    <div className={`absolute inset-0 w-full h-full bg-white flex flex-col ${getAnimationClass()}`}>
      <div className="relative h-2/5 w-full">
        {article.videoId ? <YouTubePlayer videoId={article.videoId} /> : (
          <img src={article.imageUrl} alt={article.headline} className="w-full h-full object-cover" onError={(e) => { e.target.onerror = null; e.target.src = "https://placehold.co/800x600/ccc/ffffff?text=Image+Error"; }} />
        )}
      ...
      <div className="flex-grow p-4 flex flex-col overflow-y-auto">
        ...
        <p className="text-base leading-relaxed text-gray-700 flex-grow" style={{ fontFamily: "'Noto Sans Telugu', sans-serif" }}>
          {article.body.length > 380 ? article.body.slice(0, 380) + "…" : article.body}
        </p>
        <p className="text-xs text-gray-400 mt-2">
          {new Date(article.date).toLocaleDateString("te-IN", { month: "long", day: "numeric" })} / {article.author.name}
        </p>
      </div>
      <a href={article.sourceUrl} target="_blank" rel="noopener noreferrer" className="bg-blue-600 text-center p-4 text-sm font-bold text-white hover:bg-blue-700 transition-colors cursor-pointer block z-30">
...
export default function NewsPage() {
  ...
  const [swipeDirection, setSwipeDirection] = useState('none'); // 'up', 'down', or 'none'

  const mainRef = useRef(null);
  const touchStartY = useRef(0);
  const touchEndY = useRef(0);
  
  // Ref to track the previous index for the outgoing animation
  ...
  useEffect(() => {
    ...
    const handleTouchStart = (e) => {
      // **CHANGE**: Check if touch starts on the footer link and ignore it
      if (e.target.closest('a[href]')) {
        touchStartY.current = 0; // Prevent swipe
        return;
      }
    ...
    const handleTouchMove = (e) => {
      const isInteractiveElement = ["A", "BUTTON"].includes(e.target.tagName);
    ...
    const handleTouchEnd = () => {
      if (touchStartY.current === 0) return; // Touch was ignored (e.g., on footer)
      ...
      // Reset direction after animation to prepare for the next swipe
  ...
  return (
    <div className="bg-gray-200 h-screen w-screen overflow-hidden relative">
      <Header categories={categories} selectedCategory={selectedCategory} onSelectCategory={setSelectedCategory} />
      <main ref={mainRef} className="h-full w-full relative pt-14">
        {filteredNews.length > 0 ? (
          filteredNews.map((article, index) => (
            <NewsShort key={article.id} article={article} isActive={index === currentIndex} isPrevious={index === prevIndexRef.current} swipeDirection={swipeDirection} />
          ))
        ) : (
